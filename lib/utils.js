...
utils.isSupportedSchema = function (schema) {
  return schema != null && schema.isJoi === true && (utils.isSupportedType(utils.getPrimitiveType(schema)) || utils.getMeta(schema, 'swaggerType') != null)
}
...
utils.getPrimitiveType = function (schema) {
  var swaggerType = utils.getMeta(schema, 'swaggerType')
...
utils.getPathPrefix = function (path, n) {
  n = n >= 1 ? n : 1
  return path.substr(0, path.split('/', n + 1).join('/').length)
}
