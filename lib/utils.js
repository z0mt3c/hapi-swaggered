...
utils.parseBaseModelAttributes = function (schema) {
  const required = Hoek.reach(schema, '_flags.presence') === 'required'
  const description = schema._description
  const defaultValue = Hoek.reach(schema, '_flags.default')
  const format = utils.getFormat(schema)
  const collectionFormat = utils.getMeta(schema, 'collectionFormat')
  let enumValues = Hoek.reach(schema, '_flags.allowOnly') === true ? _.filter(Hoek.reach(schema, '_valids._set'), _.isString) : undefined
...
utils.getFirstInclusionType = function (schema) {
  const inclusionTypes = Hoek.reach(schema, '_inner.inclusions')
