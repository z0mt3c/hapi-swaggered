...
utils.getLabel = function (schema) {
  return _.get(schema._flags, 'label')
}
...
utils.generateName = function (schema) {
  return utils.getLabel(schema) || utils.getMeta(schema, 'className') || utils.generateNameFromSchema(schema)
}
...
utils.parseBaseModelAttributes = function (schema) {
  ...
  let enumValues = _.get(schema, '_flags.allowOnly') === true ? _.filter([..._.get(schema, '_valids._set')], _.isString) : undefined
  const baseModel = {
    required: required
  }

  utils.setNotEmpty(baseModel, 'description', description)
  utils.setNotEmpty(baseModel, 'example', _.get(schema._examples[0], 'value'))
