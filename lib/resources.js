  ...
  prepareTags: function (path, tags, settings) {
    ...
    if (tagging.mode === 'tags') {
      if (tagging.stripRequiredTags === true) {
        return _.difference(tags, settings.requiredTags)
      }
    } else {
...
module.exports = function (settings, routes, tags) {
  ...
  var paths = _.reduce(routesByPath, function (pathsMemo, routes, path) {
    var operations = _.reduce(routes, function (operationsMemo, route) {
      ...
      var isWildcardRoute = route.method === '*'
      Hoek.assert(isWildcardRoute || _.contains(settings.supportedMethods, route.method.toLowerCase()), 'No supported http method: ' + route.method)
      var methods = !isWildcardRoute ? [route.method] : settings.supportedMethods

      _.each(methods, function (method) {
        operationsMemo[method.toLowerCase()] = operation
      })

      var exposeHead = routesPluginOptions.exposeHead === true || settings.exposeHead === true
      if (exposeHead && _.contains(methods, 'get') && operationsMemo.head == null) {
        operationsMemo['head'] = operation
      }
