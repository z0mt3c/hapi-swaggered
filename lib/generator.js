...
generator.newModel = function (schema, key, models) {
  ...
  // TODO: format key
  var modelName = utils.getMeta(schema, 'className') || Hoek.reach(schema, '_settings.className') || key || utils.generateNameFromSchema(schema)
...
generator.fromJoiSchema = function (schema, key, models) {
  ...
  // } else if (schema._type === 'date') {
    ...
    return Hoek.merge(baseModel, {
      type: utils.getMeta(schema, 'swaggerType') || Hoek.reach(schema, '_settings.swaggerType') || schema._type
    })
  }
}
